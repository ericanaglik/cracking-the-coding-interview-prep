...
various subdomains. At the top level, we have "com", at the 
next level, we have "leetcode.com", and at the lowest level, 
"discuss.leetcode.com". When we visit a domain like 
"discuss.leetcode.com", we will also visit the parent 
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        # create dictionary of subdomains
        subdomains = defaultdict(lambda: 0)
        # loop over the domains
        for cpdomain in cpdomains:
            # split on space so you have the number and the full domain
            count, domain = cpdomain.split(" ")
            # loop over the characters in the domain, once you get a period, put
            # everything to the right to the subdomains dictionary with value,
            # keep going till end
            subdomains[domain] += int(count)
            for i in range(len(domain)):
                char = domain[i]
                if char == '.':
                    subdomains[domain[i + 1:]] += int(count)
        # return as a list of all key value pairs as strings
        return [' '.join([str(value), key]) for key, value in subdomains.items()]